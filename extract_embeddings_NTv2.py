"""
Extract Nucleotide Transformer v2 (NT-v2) embeddings from the ref_seq/mut_seq
windows written by prepare_dataset.py, pooled into one .npy file.

NT-v2 (InstaDeepAI/nucleotide-transformer-v2-*-multi-species) tokenizes with a
non-overlapping 6-mer vocabulary, not one-token-per-base, so — like DNABERT-2
— downstream pooling (--pool-region downstream) uses the tokenizer's offset
mapping to translate a base position from variant_meta.csv into the token
index that first covers it. Its HF forward pass exposes only a final hidden
state (no per-layer --layer selection).
...
Input
  ...
  --model          HF model name. Default: InstaDeepAI/nucleotide-transformer-v2-50m-multi-species
                    (other sizes: 100m, 250m, 500m).
...
Output
  Saved under a folder named NTv2_{params_label}_{context_window}_emb/, e.g.
  NTv2_50M_8192bp_emb/ (context_window is parsed from the input filename —
  e.g. ref_seq_DNA_forward_8192bp.npy or variant_meta_8192bp.csv — so the
...
def parse_params_label(model_name: str) -> str:
    """Best-effort parse of the '<N>m'/'<N>M' size token in NT-v2's own HF repo
    naming convention (e.g. 'nucleotide-transformer-v2-50m-multi-species' ->
    '50M'). Falls back to the last path segment for a --model override that
    doesn't follow this convention."""
    m = re.search(r"-(\d+m)-", model_name, flags=re.IGNORECASE)
    return m.group(1).upper() if m else model_name.split("/")[-1]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract Nucleotide Transformer v2 embeddings from DNA sequences.",
...
def extract_embeddings(
    ...
    """Extract NT-v2's final hidden state, pooled per --pool-region/--emb-type,
...
if __name__ == "__main__":
    ...
    params_label = parse_params_label(args.model)
